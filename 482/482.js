 ...
 */
 const licenseKeyFormatting = function(S, K) {
    ...
    S = S.split("-").join("");
    let remainLen = S.length % K;
    if(remainLen !== 0){
        format.push(S.substring(0, remainLen));
    }
    for(let i=remainLen; i<S.length; i+=K){
        format.push(S.substring(i, i+K));
    }


    return format.join("-").toLocaleUpperCase();
};
